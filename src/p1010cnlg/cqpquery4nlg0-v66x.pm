...
sub prepareNlgFilterTemplateX4NLG{
    ...
    foreach my $el ( @nlgFilterTemplateX ){
        if ($el =~ /~/){
            ...
            my $ICountElWords = 0;
            foreach my $SWord (@LElWords){
                $strDebugX = $strDebugX . " SWord=$SWord ";
                if($ICountElWords < 1){ next; }; # first el of the array is PoS, skip
                if($SWord =~ /^[\^](.+)/){
                    $SWordBare = $1;
                    push @nlgFilterTemplateXLofLStop, $SWordBare;
                }else{
                    push @nlgFilterTemplateXLofLGo, $SWord;
                };
            ...
            push @nlgFilterTemplateXLofLStop, "[NONE]";
            push @nlgFilterTemplateXLofLGo, "[NONE]";
    
        };
    }
    
    return (\@nlgFilterTemplateXPos, \@nlgFilterTemplateXLofLStop, \@nlgFilterTemplateXLofLGo);
    # 
