...
@EXPORT = qw/create_fq_list createlist cyr2lat lat2cyr getremotehost initnum2str num2str str2num intersect min max minstring maxstring openfile signint ln llscore llcollocate micollocate tcollocate/;
...
sub create_fq_list { #parameters: word list file, [from, to]; 
    my ($fname,$fromwords,$towords,$log)=@_;
    undef $towords if (defined $towords) and ($towords eq 'last');
    my $fh=openfile($fname);
    my $curcount=0;
    if ($log) {
	$log=($log eq 'n') ? 1 : log($log);
    };
    undef my %wl;
    my $totalfq=0;
    my ($fq,$w);
    while (<$fh>) {
	if ((($fq,$w)=/^\d+\s([\d.]+)\s(.+)/) or
	    ((($fq,$w)=/^\s*([\d.]+)\s(.+)/) and ($w=~/\w/)) or # for plain fq lists from uniq -c
	    (($w,$fq)=/^(.+?)\t([\d.]+)$/) ) { # for tab-separated lists
	    $curcount++;
	    next if (defined $fromwords) and ($curcount<=$fromwords);
	    next if $fq<=0;
	    $fq=log($fq)/$log if ($log);
	    $wl{$w}+=$fq;
	    $totalfq+=$fq;
...
sub createlist { #parameters: word list file, [from, to]; 
...
sub micollocate {
    my ($onetwofrqc,$onefrqc,$twofrqc,$numwords)=@_;
    return ln($numwords * $onetwofrqc  / ($onefrqc * $twofrqc))/log(2)
}

sub tcollocate {
    my ($onetwofrqc,$onefrqc,$twofrqc,$numwords)=@_;
    my $oedifference=$onetwofrqc  - ($onefrqc * $twofrqc/$numwords);
    return ($oedifference) / sqrt ($onetwofrqc);
}
...
sub llcollocate {
    ...
    my $res=0;
    if  ($oedifference>0) {
	my $p=$twofrqc/$numwords;
	my $p1=$onetwofrqc /$onefrqc;
	my $p2=($twofrqc-$onetwofrqc)/($numwords-$onefrqc);
	$res=-0.5*(&loglike($onetwofrqc, $onefrqc, $p) + 
